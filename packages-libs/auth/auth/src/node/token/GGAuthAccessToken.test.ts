...
describe("GGAuthAccessToken", () => {
    ...
    test("the whole claim object rides under data, opaque to the engine", async () => {
        ...
        const {access} = await auth.issueAccess("user-1", {orgId: "org-9", admin: true, permissions: [Perm.Read]})
        const payload = await auth.verifyAccess(access.token)
    ...
    test("verified claims are frozen", async () => {
        const auth = accessToken()
        const {access} = await auth.issueAccess("user-1", {permissions: [Perm.Read]})
        const payload = await auth.verifyAccess(access.token)
    ...
    test("issueAccess mints an access token with no refresh counterpart", async () => {
        const auth = accessToken()
        const result = await auth.issueAccess("user-1", {permissions: [Perm.Read]})
        expect(result).not.toHaveProperty("refresh")
        expect(result.access.expiresAt).toBeGreaterThan(Date.now())
    })
    ...
    test("expired access token is rejected as TOKEN_EXPIRED", async () => {
        ...
        const {access} = await auth.issueAccess("user-1", {permissions: [Perm.Read]})
        await expectAuthError(auth.verifyAccess(access.token), "TOKEN_EXPIRED")
    })
    ...
    test("token minted for one audience is rejected by an instance with a different audience", async () => {
        ...
        const {access} = await userAuth.issueAccess("user-1", {permissions: [Perm.Read]})
        await expectAuthError(orgAuth.verifyAccess(access.token), "TOKEN_INVALID")
    })
})
