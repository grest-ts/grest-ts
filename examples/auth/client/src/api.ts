...
export const session = GGAuthSession.withToken(USER_TOKEN, {
    refresh: async (token) => api.authApi.refresh({refreshToken: token}),
}).addDerived("org", ORG_TOKEN, {
    mint: async (data: SelectOrgRequest) => {
